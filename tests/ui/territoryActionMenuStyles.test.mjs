...
test("segmented territory menu keeps icons visible inside fixed quadrants", () => {
  ...
  assert.match(styles, /\.territory-action-info-ring\s*{[^}]*inset: -38px;[^}]*background:[^;]*conic-gradient\([^;]*0deg 72deg,[^;]*72deg 144deg,[^;]*144deg 216deg,[^;]*216deg 288deg,[^;]*288deg 360deg[^;]*\);[^}]*animation: territory-info-slide-out 180ms ease-out both;/s);
