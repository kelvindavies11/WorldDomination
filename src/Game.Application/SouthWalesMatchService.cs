...
public sealed class SouthWalesMatchService
{
    ...
    private readonly SouthWalesPostcodeTerritoryRepository postcodeTerritoryRepository;
    private readonly WalesTerritoryFeatureRepository territoryFeatureRepository;
    private readonly Random random;

    public SouthWalesMatchService(GameMapService mapService)
        : this(mapService, new SouthWalesPostcodeTerritoryRepository(), new WalesTerritoryFeatureRepository(), Random.Shared)
    {
    }

    public SouthWalesMatchService(
        GameMapService mapService,
        SouthWalesPostcodeTerritoryRepository postcodeTerritoryRepository,
    ...
    public MatchSnapshot CreateSouthWalesLobbyMatch(MatchSetupOptions setup)
    {
        var map = mapService.GetMap("south-wales");
    ...
    private static IReadOnlyDictionary<string, int> CreateStartIndexes(int territoryCount, IReadOnlyList<string> factionIds, Random random)
    {
        ...
        {
            throw new InvalidOperationException($"South Wales postcode territory data must contain at least {factionIds.Count} territories.");
        }
