...
public sealed class NorthWalesMatchService
{
    ...
    private readonly NorthWalesPostcodeTerritoryRepository postcodeTerritoryRepository;
    private readonly WalesTerritoryFeatureRepository territoryFeatureRepository;
    private readonly Random random;

    public NorthWalesMatchService(GameMapService mapService)
        : this(mapService, new NorthWalesPostcodeTerritoryRepository(), new WalesTerritoryFeatureRepository(), Random.Shared)
    {
    }

    public NorthWalesMatchService(
        GameMapService mapService,
        NorthWalesPostcodeTerritoryRepository postcodeTerritoryRepository,
    ...
    public MatchSnapshot CreateNorthWalesLobbyMatch(MatchSetupOptions setup)
    {
        var map = mapService.GetMap("north-wales");
    ...
    private static IReadOnlyDictionary<string, int> CreateStartIndexes(int territoryCount, IReadOnlyList<string> factionIds, Random random)
    {
        ...
        {
            throw new InvalidOperationException($"North Wales postcode territory data must contain at least {factionIds.Count} territories.");
        }
