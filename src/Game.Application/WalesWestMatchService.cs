...
public sealed class WalesWestMatchService
{
    ...
    private readonly WalesWestPostcodeTerritoryRepository postcodeTerritoryRepository;
    private readonly WalesWestTerritoryFeatureRepository territoryFeatureRepository;
    private readonly Random random;

    public WalesWestMatchService(GameMapService mapService)
        : this(mapService, new WalesWestPostcodeTerritoryRepository(), new WalesWestTerritoryFeatureRepository(), Random.Shared)
    {
    }

    public WalesWestMatchService(
        GameMapService mapService,
        WalesWestPostcodeTerritoryRepository postcodeTerritoryRepository,
        WalesWestTerritoryFeatureRepository territoryFeatureRepository,
        Random? random = null)
    {
    ...
    public MatchSnapshot CreateWalesWestLobbyMatch(MatchSetupOptions setup)
    {
        var map = mapService.GetMap("wales-west");
    ...
    private static IReadOnlyDictionary<string, int> CreateStartIndexes(int territoryCount, IReadOnlyList<string> factionIds, Random random)
    {
        ...
        {
            throw new InvalidOperationException($"Wales West postcode territory data must contain at least {factionIds.Count} territories.");
        }
